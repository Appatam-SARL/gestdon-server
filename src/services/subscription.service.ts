...
export class SubscriptionService {
  ...
  static async confirmPayment(
    ...
    try {
      ...
      await this.activateContributorSubscription(
        String(subscription.contributorId),
        subscriptionId,
        package_,
        session
      );
  ...
  private static async activateContributorSubscription(
    ...
    contributor.status = 'active' as ContributorStatus;
  ...
  static async renewSubscription(subscriptionId: string): Promise<ApiResponse> {
    try {
      ...
      const renewalData = {
        contributorId: subscription.contributorId as mongoose.Types.ObjectId,
        packageId: subscription.packageId._id as mongoose.Types.ObjectId,
        paymentMethod: subscription.paymentMethod,
        autoRenewal: subscription.autoRenewal,
      };

      return await this.createSubscription(
        renewalData as unknown as CreateSubscriptionRequest
      );
    } catch (error) {
      return {
        success: false,
  ...
  // Créer une souscription gratuite (essai gratuit)
  static async createFreeTrialSubscription(
    ...
    if (await supportsTransactions) {
      // Utiliser les transactions si supportées
      return this.createFreeTrialWithTransaction(contributorId, packageId);
    } else {
      // Fallback sans transaction
      return this.createFreeTrialWithoutTransaction(contributorId, packageId);
    }
  }

  // Version avec transaction
  private static async createFreeTrialWithTransaction(
    contributorId: string,
    packageId: string
  ): Promise<ApiResponse> {
    const session = await mongoose.startSession();
    ...
    try {
      ...
      // Vérifier que le package existe et est gratuit
      const package_ = await PackageModel.findOne({
        _id: packageId,
        isActive: true,
        isFree: true,
      ...
      if (!package_) {
        throw new Error('Package gratuit non trouvé ou inactif');
      }
      ...
      // Vérifier si le contributeur a déjà utilisé un essai gratuit
      const existingFreeTrial = await SubscriptionModel.findOne({
        contributorId,
        isFreeTrial: true,
      }).session(session);

      if (existingFreeTrial) {
        throw new Error('Le contributeur a déjà utilisé un essai gratuit');
      }

      // Calculer la date de fin (1 mois par défaut ou selon maxFreeTrialDuration)
      const startDate = new Date();
      const endDate = new Date();
      const trialDuration = package_.maxFreeTrialDuration || 30; // 30 jours par défaut

      endDate.setDate(startDate.getDate() + trialDuration);

      // Créer la souscription gratuite
      const subscription = new SubscriptionModel({
        contributorId,
        packageId,
        startDate,
        endDate,
        status: SubscriptionStatus.ACTIVE,
        paymentStatus: PaymentStatus.PAID, // Pour les essais gratuits
        amount: 0,
        currency: 'XOF',
        autoRenewal: false, // Les essais gratuits ne se renouvellent pas
        isFreeTrial: true,
        transactionId: `FREE_TRIAL_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`,
      });
  ...
  // Version sans transaction (fallback)
  private static async createFreeTrialWithoutTransaction(
    contributorId: string,
    packageId: string
  ): Promise<ApiResponse> {
    try {
      ...
      const startDate = new Date();
      const endDate = new Date();
      const trialDuration = package_.maxFreeTrialDuration || 30;
      endDate.setDate(startDate.getDate() + trialDuration);
  ...
  // Méthode pour gérer automatiquement les abonnements expirés
    ...
    try {
      ...
      for (const subscription of expiredSubscriptions) {
        try {
          ...
          // Si c'était un essai gratuit, désactiver le renouvellement automatique
          ...
          result.errors.push(
            `Erreur lors du traitement de l'abonnement ${subscription._id}: ${error}`
          );
        }
        result.processed++;
      }

      // Traiter les abonnements qui expirent bientôt (notifications)
      ...
      // aux contributeurs dont l'abonnement expire bientôt
    } catch (error) {
  ...
  // Méthode pour vérifier si un contributeur peut souscrire à un essai gratuit
  static async canSubscribeToFreeTrial(
    contributorId: string
  ): Promise<boolean> {
    try {
      const existingFreeTrial = await SubscriptionModel.findOne({
      ...
      return !existingFreeTrial;
    } catch (error) {
      return false;
    }
  }

  // Méthode pour vérifier le statut de l'abonnement d'un utilisateur
  static async checkUserSubscriptionStatus(
    ...
    try {
      ...
      const activeSubscription = await SubscriptionModel.findOne({
        ...
        status: SubscriptionStatus.ACTIVE,
        endDate: { $gt: new Date() },
        $or: [{ paymentStatus: PaymentStatus.PAID }, { isFreeTrial: true }],
      }).populate('packageId');
      ...
      return {
        ...
        data: {
          ...
          contributor: user.contributorId,
          user: {
            id: user._id,
            email: user.email,
